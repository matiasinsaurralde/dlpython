...
func PyGILStateEnsure() unsafe.Pointer {
	// state := PyGILState_Ensure()
	// return unsafe.Pointer(&state)
	fmt.Println("*** GIL Ensure")
	C.gilState = PyGILState_Ensure()
	return nil
}

func PyGILStateEnsure2() interface{} {
	state := PyGILState_Ensure()
	return state
}

func PyGILStateRelease2(state interface{}) {
	// st := ToPyGILState_STATE(state)
	// PyGILState_Release(st)
}

func PyGILStateRelease() {
	// PyGILState_Release(state.(C.PyGILState_STATE))
	// s := (*C.PyGILState_STATE)(state)
	// PyGILState_Release(*s)
	fmt.Println("*** GIL Release")
	PyGILState_Release(C.gilState)
}

func PyEvalInitThreads() {
	PyEval_InitThreads()
}

func PyEvalSaveThread() {
	PyEval_SaveThread()
}

/*
func ToPyGILState_STATE(s interface{}) C.PyGILState_STATE {
	state := s.(C.PyGILState_STATE)
	return state
}
